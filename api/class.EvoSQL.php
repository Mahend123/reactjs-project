  ...
  private $db_host = "localhost";
	...
	public function Connect($db_host = null, $db_name = null, $db_user = null, $db_pass = null) {
      ...
      // Open Database Connection
		$this->sql_link=@mysql_connect ($this->db_host, $this->db_user, $this->db_pass) or die("<pre>" . print_r($this, 1) . '<br />Cannot connect to the database because: ' . mysql_error());
		if (!$this->sql_link) {
	...
	public function MakeWhereQuery($whereArray) {
	...
	public function RowCount($tableName, $whereArray = 0) {
		if (! is_array($whereArray) && $whereArray != 0) die('whereArray is not an array in RowCount');
		$query = "select count(*) as TheCount from " . $tableName . $this->MakeWhereQuery($whereArray);
